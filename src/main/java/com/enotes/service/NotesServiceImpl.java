...
public class NotesServiceImpl implements NotesService {
	 ...
	 * //Save notes
	 * 
	 * @Override public boolean saveNotes(NotesDto notesDto) throws Exception { //
	 ...
	 * 
	 * //First convert notesdto to notes //Notes notes = modelMapper.map(notesDto,
	 * Notes.class);
	 * 
	 * 
	 * //Manual Notes notes = new Notes(); notes.setTitle(notesDto.getTitle());
	 * notes.setDescription(notesDto.getDescription());
	 * 
	 * // ✅ MANUAL RELATIONSHIP MAPPING (FIX) if (notesDto.getCategory() != null &&
	 * notesDto.getCategory().getId() != null) { Category category = new Category();
	 * category.setId(notesDto.getCategory().getId()); notes.setCategory(category);
	 * }
	 * 
	 * 
	 * //Save in database Notes saveNotes = notesRepository.save(notes);
	 * 
	 * //Check notes are empty or not if(!ObjectUtils.isEmpty(saveNotes)) { return
	 * true; }
	 * 
	 * return false; }
	 */
	...
	public boolean saveNotes(String notes, MultipartFile file) throws Exception {
		...
		/*
		 * //Save file details FileDetails fileDetails = saveFileDetails(file);
		 * 
		 * if(ObjectUtils.isEmpty(fileDetails)) { notesobj.setFileDetails(fileDetails);
		 * } else { notesobj.setFileDetails(null); }
		 */
	...
	public List<NotesDto> getAllNotes() {
		...
		// List<NotesDto> notesDtoList = notes.stream().map(note ->
		// modelMapper.map(note, NotesDto.class)).toList();
	...
	public void favouriteNotes(Integer noteId) throws Exception {
		...
		FavouritNote favouritNote = FavouritNote.builder().note(notes).userId(userId).build();

		favouriteNoteRepository.save(favouritNote);
	...
	public List<FavouritNoteDto> getUserFaouriteNotes() throws Exception {
		...
		List<FavouritNoteDto> favouriteList = favouriteNotes.stream().map(fn -> 
		modelMapper.map(fn, FavouritNoteDto.class)).toList();
